...
class QueueUsingStacks:
    ...
    def dequeue(self):
        while self.pilha_principal.is_empty() == False:
            top = self.pilha_principal.peek()
            self.pilha_aux.push(top)
            self.pilha_principal.pop()

        removed = self.pilha_aux.pop()
        while self.pilha_aux.is_empty() == False:
            top = self.pilha_aux.peek()
            self.pilha_principal.push(top)
            self.pilha_aux.pop()
